...
function exTextAnimation() {
    informationLabel = createLabel();
    loadWebFont(beginTextAnimation);   
}

function loadWebFont(callback) {
    var webFontName = "Limelight";
    WebFont.load({
		google:{
			families:[webFontName]
		},
		loading:function(){
			setInformationLabel("WebFont\n Loading Now!");
		},
		active:function(){
			//読み込み成功
			setInformationLabel("WebFont\n Loading END");
			callback(true, webFontName);
		},
		inactive:function(){
			//読み込み失敗
			setInformationLabel("WebFont None!");
			callback(false, "Arial");
		}
	});
}

function beginTextAnimation(isFontLoadSuccess, fontName) {
    ...
    setInformationLabel("XXXXX\nYYYYYY");
    setTextTween(informationLabel);
    
    animationChars = createTextArray("This is CreateJS world", "32px " + fontName, 5, "#882233");
    
    for(var i = 0; i < animationChars.length; i++) {
        stage.addChild(animationChars[i]);    
    }
...
function createLabel() {
    ...
    mask.graphics.beginFill("#000").drawRect(0,0,300,300);
    container.mask = mask;  
    
    var shadow=new Shadow("#000000",0,0,4);
    var label = new Text("","12px Arial","#FFFFFF");
	label.x = 20;
	label.y = 15;
	label.maxWidth = 300;
	label.lineHeight = 20;
...
function setInformationLabel(str) {
    informationLabel.text = str;
    stage.update();
}

function createTextArray(text, font, space, color) {
    var array = [];
    var x = 0;
    for(var i = 0; i < text.length; i++ ) {
        var c = new Text("", font, color);
        c.text = text.charAt(i);
        var width  = c.getMeasuredWidth();
		var height = c.getMeasuredHeight();
        c.width = width;
        c.height = height;
        c.regX = width / 2;
        c.regY = height / 2;
...
function setTextTween(target) {
    var tween = new Tween(target, {loop: true});
    tween.to({x: 300, y: 300 - 20, rotation: 360, alpha: 0.3}, 5000, Ease.bounceOut);
    tween.wait(100);
